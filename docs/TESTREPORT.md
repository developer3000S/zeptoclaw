...
- **arm64-оборудование**: кросс-сборка зелёная, прогонов на arm64-машине не было.
- **Реальный бинарник PicoClaw**: интеграция покрыта kontrakным CLI-тестом на
...
- CI-конвейер как таковой ([15.1 conveyor]) не настроен — гейты раздела 5
  прогоняются вручную/скриптом; `make bench` объявлена, но файлов-мишеней нет
  (замеры маршрутизации/канонизации не проводились).
