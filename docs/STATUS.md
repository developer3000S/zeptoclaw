...
- [доп. к 6.3] Активный скан окружения агентом: `internal/node/scan.go` +
  `internal/discovery/scan.go`. По запросу оператора (`POST /api/v1/admin/scan`,
  CLI `zeptomesh-node scan`) или медленному фоновому циклу
  (`discovery.scan.enabled`) узел переспрашивает все источники (локальный
  реестр, mDNS, DHT по своим навыкам, peer-exchange, bootstrap, search topic) и
  опционально перебирает свою локальную подсеть (`discovery.scan.subnet_scan`),
  находя агентов, которые ещё не объявились; каждый найденный проходит dial +
  проверка подписанных capabilities и становится полноправным соседом.
  Ограничения: бюджет скана, лимит кандидатов, политика безопасности
  (`Policy.AllowConnection` — заблокированный пир не дозванивается),
  перебор подсети по умолчанию выключен. Тесты: `internal/discovery/scan_test.go`,
  `internal/config/scan_test.go`, `internal/node/integration_test.go`
  (`TestIntegrationScanLocal`, `TestIntegrationScanBlockedPeer`).
...
| `internal/config/scan_test.go` | секция `discovery.scan`: фон по умолчанию выключен, перебор подсети по умолчанию выключен, позитивные длительности/лимит, полное чтение включённого блока, отказ при `enabled` с нулевым `interval`, при `dial_timeout > timeout` и при отрицательном `max_candidates`; изменение секции — `requires_restart` |
| `internal/discovery/scan_test.go` | активный скан на живом libp2p: `MeshTCPPorts` извлекает только TCP-порты (коллапс дублей, пропуск udp/quic, мусор и пустой список безопасны), `PrivateSubnets` отдаёт только RFC1918 /24, `SubnetSweep` находит открытый mesh-порт слушающего хоста на loopback (параллельный пул, nil-хост/подсеть/порты — no-op без паники) |
