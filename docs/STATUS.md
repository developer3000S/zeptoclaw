...
- [15.1] **arm64 проверена без чужеродного рантайма на хосте**: `make cross`
  ...
  производительность и не гонки. CI-конвейера нет — проверка однократная,
  вручную.
...
4. Конфигурируемое пространство имён метрик (вопрос §6.2), CI-конвейер
   [15.1] (сама arm64-проверка выполнена — см. §1; в конвейере её нет).
